...
import atexit
import jsonpickle
from INITIAL_ATTEMPS_ignore.OKAY_SECOND_ATTEMPT.player_moves import ThePlayer
from INITIAL_ATTEMPS_ignore.OKAY_SECOND_ATTEMPT.ships_status import Ship, ShipElement
from INITIAL_ATTEMPS_ignore.OKAY_SECOND_ATTEMPT.gameover import GameOverScreen
...
def clear_request():
    ...
    :return:
    """
    players = App.get_running_app().game
    # if players is an instance of ThePlayer, send a clear request
    if len(players) >= 2:
        if isinstance(players[1], ThePlayer):
