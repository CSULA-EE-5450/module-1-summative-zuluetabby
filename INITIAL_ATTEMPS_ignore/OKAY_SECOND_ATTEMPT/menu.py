...
from INITIAL_ATTEMPS_ignore.OKAY_SECOND_ATTEMPT.battleship_game import multiplayer_init, player_vs_mode
from INITIAL_ATTEMPS_ignore.OKAY_SECOND_ATTEMPT.battleship_game import Battleship
from INITIAL_ATTEMPS_ignore.OKAY_SECOND_ATTEMPT.player import PlayerScreen
from INITIAL_ATTEMPS_ignore.OKAY_SECOND_ATTEMPT.player_moves import Player, ThePlayer
...
class MenuButtonAnimation(Animation):
    ...
    def on_complete(self, widget):
        if widget.player1 and widget.player2:
            initialize_game(widget.player1, widget.player2, widget.screens)
        else:
            if widget.screens is True:
                widget.screens()
            screen_manager = App.get_running_app().screen_manager
